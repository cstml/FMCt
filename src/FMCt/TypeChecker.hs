...
derive2Aux term = do
    ...
    where
        ...
        replaceInfer ::
            [T] ->
            Term ->
            -- | Return a Tuple formed out of the new pre-processed term and the stream left.
            (Term, [T])
        ...
        chkUnique x =
            if length x == length (nub $ fmap fst x)
                then pure x
...
merge ::
    -- | Substitutions to be made in both types.
    [TSubs] ->
    -- | The consuming Type.
    T ->
    -- | The merged Type.
    T ->
    -- | The result containing: (new list of substitutions,
    -- unmerged types remaining from the consuming type,
    -- unmerged types remaining from the merged type).
    ([TSubs], T, T)
merge exSubs x y =
    let x' = normalForm . normaliseT . (applyTSub exSubs) $ x -- we use the already subtituted form when consuming
     ...
     in case x' of
            ...
            TLoc xl' xt' -> case y' of
                ...
                TLoc yl' yt' ->
                    if xl' == yl'
                        then (finalSubs, TLoc xl' finalX', TLoc yl' finalY')
                        else (exSubs, x', y')
                    where
                ...
                iy' :=> oy' ->
                    if x'' == y''
                        then (exSubs, mempty, mempty)
                            ...
                            if (finalSubs, finalL, finalR) == (finalSubs, TEmp, TEmp)
