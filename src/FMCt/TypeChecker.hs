...
consumes :: T                     -- ^ Requires - What the term will consume
         -> T                     -- ^ Receives - what the term is getting
         ->  Either TError (T, T) -- ^ (Left from Requires, Left from Receives)
consumes = \case
    TCon "" -> pure . (,) mempty
    TVec [] -> pure . (,) mempty
    xx@(TCon x) -> \case
        ...
        y@(TLoc l' t') ->
            if l == l'
                then case consumes t t' of
                    Left e            -> Left e
                    Right (res, left) -> pure ( normaliseT $ TLoc l res
    ...
    x@(_ :=> _) -> \case
        ...
        y@(_ :=> _) -> 
          let
            ...
            y' = normaliseT y
          in
            if x' == y' then pure (mempty, mempty) else pure (x', y')
